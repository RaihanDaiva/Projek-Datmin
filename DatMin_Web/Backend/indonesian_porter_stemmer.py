...
class IndonesianPorterStemmer:
    ...
    def stem_tokens(self, tokens):
        ...
        Args:
            tokens (list): List token hasil tokenizing / filtering / case folding
            
        Returns:
            list: List token hasil stemming
        """
        if not isinstance(tokens, list):
            raise TypeError("Input harus berupa list token")

        stemmed_tokens = []
        for token in tokens:
            if isinstance(token, str) and token.isalpha():
                stemmed_tokens.append(self.stem(token))
            else:
